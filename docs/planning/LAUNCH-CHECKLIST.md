...
| L-4 | TEST environment badge + export watermark | **DONE** | Discreet corner chip when the account is `is_test` or `VITE_ENV` is a non-prod value; TEST watermark + "not for fulfillment" note on PDF/CSV export of `is_test` orders. |
