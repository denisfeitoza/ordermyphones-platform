-- 20260807230000_sku_include_color.sql
-- SKU collision fix surfaced by the live 2,675-row HYLA import.
--
-- Root cause: product_variants carries TWO unique constraints —
--   * product_variants_natural_key: UNIQUE (product_id, capacity,
--     COALESCE(color,''), carrier, lock_status, grade)  ← INCLUDES color
--   * product_variants_sku_key:     UNIQUE (sku)
-- but omp_make_sku(model, capacity, grade, carrier, lock) did NOT include
-- color. Two variants that differ ONLY by color (e.g. Pixel 8 128GB DLS B OTH
-- LOCKED in Black vs White) are two distinct natural-key rows, yet computed
-- the SAME sku → the second insert violates product_variants_sku_key and the
-- whole import transaction aborts.
--
-- Fix: color becomes part of the SKU so it is unique per natural-key row. The
-- canonical SKU is now MODEL-CAP-COLOR-GRADE-CARRIER-LOCK — a COLOR token is
-- inserted right AFTER capacity. commit_stock_import is re-authored identical
-- to 20260807160000 (reprice hook preserved) EXCEPT the single omp_make_sku
-- call site in the distinct_variants insert, which now passes color.
--
-- Policy call convention unchanged: helper is SECURITY DEFINER, owner
-- postgres, search_path = '', immutable, EXECUTE revoked from
-- public/anon/authenticated (only commit_stock_import calls it, as the owner).
--
-- D16 (repo is PUBLIC): no real supplier legal name anywhere in this file.

------------------------------------------------------------------
-- A. public.omp_make_sku — deterministic canonical SKU, now color-aware.
...
comment on function public.omp_make_sku(text, text, text, text, public.carrier_code, public.lock_status) is
  'Deterministic canonical SKU MODEL-CAP-COLOR-GRADE-CARRIER-LOCK. Color is part of the key so two variants differing only by color get distinct SKUs (product_variants_sku_key). Algorithm mirrored byte-for-byte in apps/web/src/lib/import/sku.ts — keep both in lockstep.';

-- Drop the old 5-arg overload: it computed a color-blind SKU that collides
-- across colors, and nothing should call it anymore. commit_stock_import
-- (re-authored below) calls only the new 6-arg form.
drop function if exists public.omp_make_sku(text, text, text, public.carrier_code, public.lock_status);
...
revoke execute on function public.omp_make_sku(text, text, text, text, public.carrier_code, public.lock_status) from public, anon, authenticated;

------------------------------------------------------------------
-- B. public.commit_stock_import — re-authored byte-identical to
-- 20260807160000_commit_reprice_hook.sql EXCEPT the one omp_make_sku call
-- site in the distinct_variants insert, which now passes color. The reprice
-- hook (perform public.reprice_variants(...)) and every other line are
-- preserved verbatim.
------------------------------------------------------------------
...
begin
  ...
  select product_id, capacity, color, carrier, carrier_raw, lock_status, grade, grade_scale,
         public.omp_make_sku(model, capacity, color, grade, carrier, lock_status)
