-- 20260913130100_commit_stock_import_lock.sql
-- commit_stock_import from 20260807240000_sku_model_number.sql, byte-for-byte,
-- plus ONE addition: the touched inventory rows are locked FOR UPDATE before
-- the delta computation (audit 2026-09-13 P1-13). Nothing else changes.
...
begin
  ...
  ------------------------------------------------------------------
  -- Lock the inventory rows this import touches BEFORE computing deltas
  -- (audit 2026-09-13 P1-13): approve_order locks the same rows FOR UPDATE,
  -- so without this an approval that lands mid-import would make the
  -- "sheet minus current qty" delta stale and the final balance wrong.
  perform 1
  from public.inventory i
  join tmp_import_grouped g on g.variant_id = i.variant_id and g.location_id = i.location_id
