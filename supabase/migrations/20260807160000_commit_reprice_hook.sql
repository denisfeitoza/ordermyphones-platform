-- 20260807160000_commit_reprice_hook.sql
-- Post-import auto-repricing hook for public.commit_stock_import.
--
-- 20260807150000 (pricing_engine) introduced public.reprice_variants(uuid[])
-- as the waterfall engine's entry point, and its own comment already
-- anticipated this exact wiring: "The orchestrator's post-import reprice
-- hook will call this from inside commit_stock_import (or an equivalent
-- postgres-owned SECURITY DEFINER caller) in a later migration — that call
-- works with zero additional grants." This migration is that hook.
--
-- commit_stock_import is SECURITY DEFINER owned by postgres (see
-- 20260807140000), and reprice_variants revokes EXECUTE from
-- public/anon/authenticated but leaves it implicitly available to the
-- postgres-owned function that calls it — same pattern already used for the
-- four omp_* import helpers. No new grants needed here.
--
-- Body is byte-identical to 20260807140000's commit_stock_import EXCEPT for
-- one new statement: immediately after the inventory cost/as_of upsert and
-- before the movements_written count, every variant touched by this import
-- (distinct variant_id across tmp_import_grouped) is repriced.
--
