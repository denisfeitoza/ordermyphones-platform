...
export function RealDashboard() {
  ...
  return (
    <div className="space-y-6">
      <AdminHeading title={t('Dashboard')} subtitle={t('Live operational snapshot — real catalog, orders and stock.')} />
      ...
      <section className="rounded-2xl border border-brand/30 bg-brand/5 p-4 sm:p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-sm font-semibold">{t('View the store as a customer')}</h2>
              <p className="text-xs text-muted-foreground">{t('Opens the storefront priced for that tier — exactly what the customer sees.')}</p>
            </div>
          </div>
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6">
        <StatCard label={t('SKUs live')} value={s ? formatInt(s.skusLive) : '—'} sub={t('priced & in stock')} live />
        <StatCard label={t('Customers')} value={s ? formatInt(s.customers) : '—'} sub={t('registered')} />
        <StatCard label={t('Orders')} value={formatInt(orders.length)} sub={t('all-time')} />
        <StatCard label={t('Pending approval')} value={formatInt(pending)} sub={t('awaiting staff')} accent={pending > 0 ? 'text-warning' : 'text-foreground'} />
        <StatCard label={t('Reconciliations')} value={formatInt((reconQ.data ?? []).length)} sub={t('open shortfalls')} accent={(reconQ.data ?? []).length > 0 ? 'text-warning' : 'text-foreground'} />
        <StatCard label={t('Pricing flags')} value={s ? formatInt(s.openFlags) : '—'} sub={t('need review')} accent={(s?.openFlags ?? 0) > 0 ? 'text-warning' : 'text-foreground'} />
      </div>

      <div className="grid gap-6 lg:grid-cols-3 [&>*]:min-w-0">
        <section className="space-y-3 lg:col-span-2">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-medium">{t('Recent orders')}</h2>
          ...
          {recent.length > 0 ? (
            <Table
              columns={[
                { key: 'id', label: t('Order') },
                { key: 'customer', label: t('Customer') },
                { key: 'units', label: t('Units'), align: 'right' },
            ...
            <div className="rounded-2xl border border-dashed border-border bg-card p-10 text-center text-sm text-muted-foreground">
              {t('No orders yet. As customers place orders they appear here and in the approval queue.')}
            </div>
          )}
        </section>

        <div className="space-y-3">
          <h2 className="text-sm font-medium">{t('Quick actions')}</h2>
          <div className="grid gap-3">
            <QuickLink to="/admin/import" icon={<UploadCloud className="h-4 w-4" strokeWidth={2} />} title={t('Import stock')} sub={t('Upload a supplier sheet')} />
            <QuickLink to="/admin/prices" icon={<Tag className="h-4 w-4" strokeWidth={2} />} title={t('Set prices')} sub={t('Consumer/retailer benchmarks')} />
            <QuickLink to="/admin/orders" icon={<Receipt className="h-4 w-4" strokeWidth={2} />} title={t('Review orders')} sub={t('Approve & reconcile')} />
          </div>
        </div>
      </div>
    </div>
